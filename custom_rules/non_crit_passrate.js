/** @type {import("allure/rules).QualityGateRule<number, [number, number]>} */
export const passRateNonCriticalsRule = {
  rule: "passRateNonCriticals",
  message: ({ actual, expected }) =>
    `The pass rate of tests with non-critical severity label ${actual} is less than ${expected}`,
  validate: async ({ trs, expected, state }) => {
    console.log("STATE: ", state);

    // find all tests with the critical severity label
    const withSeverities = trs.filter((tr) =>
      tr.labels.some(
        (label) =>
          label.name === "severity" &&
    ...
    const withSeveritiesPass = withSeverities.filter(
      (tr) => tr.status === "passed",
    );
